...
mutdata2 = mutdata2[,colnames(mutdata2) != "WT1"]
...
colnames(mydf) = c("gene","beta_WT1gain","beta_WT1gain_lowCI","beta_WT1gain_hiCI","pval_WT1gain","beta_WT1loss","beta_WT1loss_lowCI","beta_WT1loss_hiCI","pval_WT1loss","beta_WT1shared","beta_WT1shared_lowCI","beta_WT1shared_hiCI","pval_WT1shared")

write.table(mydf,"WT1-binary-gain-vs-loss.associatedMuts.logistf.txt",sep="\t",row.names=FALSE)
...
#WT1 gain
g1= ggplot(mydf2,aes(x=beta_WT1gain,y=gene)) + geom_errorbar(aes(xmin=beta_WT1gain_lowCI, xmax=beta_WT1gain_hiCI),width=0.1,size=0.4) + geom_point(size=5,color="#ED5D00")+ theme_pubr()+geom_vline(xintercept=0,linetype=2,color="gray44") +rremove("legend") + scale_color_distiller(palette="BrBG") + geom_text(aes(x=0.2,y=gene,label=signif(pval_WT1gain,digit=3)),nudge_y=0.2)

#WT1 loss
#mydf2 = mydf2[order(mydf2$beta_WT1loss,decreasing=TRUE),]
#mydf2$gene = factor(mydf2$gene,levels=rev(mydf2$gene))
g2= ggplot(mydf2,aes(x=beta_WT1loss,y=gene)) +  geom_errorbar(aes(xmin=beta_WT1loss_lowCI, xmax=beta_WT1loss_hiCI),width=0.1,size=0.4) + geom_point(size=5,color="#0095A4") + theme_pubr()+geom_vline(xintercept=0,linetype=2,color="gray44") +rremove("legend") + geom_text(aes(x=0.2,y=gene,label=signif(pval_WT1loss,digit=3)),nudge_y=0.2)

#WT1 shared
g3= ggplot(mydf2,aes(x=beta_WT1shared,y=gene)) +  geom_errorbar(aes(xmin=beta_WT1shared_lowCI, xmax=beta_WT1shared_hiCI),width=0.1,size=0.4) + geom_point(size=5,color="#0095A4") + theme_pubr()+geom_vline(xintercept=0,linetype=2,color="gray44") +rremove("legend") + geom_text(aes(x=0.2,y=gene,label=signif(pval_WT1shared,digit=3)),nudge_y=0.2)
