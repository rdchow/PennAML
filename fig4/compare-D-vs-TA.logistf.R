...
data = read.table("AML_data.muts.chroms.matrix.inductOnly.txt",sep="\t",header=TRUE,row.names=1)
data = data[,colnames(data) != "TPMT"]

info = read.table("../fig2/uniqueStage-sample-names.inductOnly.txt",sep="\t")
...
# can only have TET2/ASXL1 or DNMT3A mutations; mutually exclusive
dataDN$TET2ASXL1 = pmax(dataDN$TET2,dataDN$ASXL1)
dataDN2 = dataDN[dataDN$TET2ASXL1 + dataDN$DNMT3A == 1 ,]  # 26 vs 27
dataDN2$DTA = dataDN2$DNMT3A - dataDN2$TET2ASXL1 # -1: TET2/ASXL1; +1: DNMT3A

library(logistf)
mydf = matrix(nrow = ncol(dataDN2)-4,ncol=5)
for (i in c(1:(ncol(dataDN2)-4))){
...
colnames(mydf) = c("gene","beta_DNMT3A_vs_TET2ASXL1","lowCI","hiCI","pval")
mydf = as.data.frame(mydf)
mydf = mydf[! mydf$gene %in% c("DNMT3A","TET2","ASXL1"),]
mydf = mydf[order(mydf$pval),]

write.table(mydf,"DNMT3A-vs-TET2ASXL1.exclusive.diagnosis.logistf.inductOnly.txt",sep="\t",row.names=FALSE)
...
datat = dataDN2[dataDN2$DTA == -1,] # TET2/ASXL1

d1 = cbind(rownames(datad),datad$patient,datad$stage,"DNMT3Amut")
d2 = cbind(rownames(datat),datat$patient,datat$stage,"TET2ASXL1mut")
sampled2 = as.data.frame(rbind(d1,d2))
write.table(sampled2,"diagnosis.any.DNMT3A-TET2ASXL1.samples.inductOnly.txt",sep="\t",row.names=FALSE)

dmut = colSums(datad[,1:(ncol(datad)-4)])
dwt = nrow(datad) - dmut
dd = cbind(dmut,dwt)

tmut = colSums(datat[,1:(ncol(datat)-4)])
twt = nrow(datat) - tmut
tt = cbind(tmut,twt)
...
data = read.table("DNMT3A-vs-TET2ASXL1.exclusive.diagnosis.logistf.inductOnly.txt",sep="\t",header=TRUE,row.names=1)
...
pdf("diagnosis.DNMT3A-vs-TET2ASXL1.mutfreqs.logistf.pdf",height=5,width=4,useDingbats=FALSE)
...
geom_point(data=data[data$pval <0.05,],size=1.5,stroke=0.8,pch=8,fill=NA) + 
...
labels = read.table("diagnosis.any.DNMT3A-TET2ASXL1.samples.inductOnly.txt",sep="\t",header=TRUE)
...
dataREL[dataREL$DTA == "TET2ASXL1mut","DTA"] = -1
...
write.table(mydf,"DNMT3A-vs-TET2ASXL1.exclusive.REL1.logistf.inductOnly.txt",sep="\t",row.names=FALSE)
...
data = read.table("DNMT3A-vs-TET2ASXL1.exclusive.REL1.logistf.inductOnly.txt",sep="\t",header=TRUE,row.names=1)
fdata = fdata[rownames(fdata) %in% rownames(data),]
fdata = fdata[match(rownames(data),rownames(fdata)),]
all(rownames(fdata) == rownames(data))
...
pdf("REL1.DNMT3A-vs-TET2ASXL1.mutfreqs.logistf.pdf",height=5,width=4,useDingbats=FALSE)
ggscatter(data,x="d_mutfreq", y="t_mutfreq",fill="sumPct",shape=21,color="black",repel=TRUE,alpha=0.75,size="nlp",xlim=c(0,80),ylim=c(0,40),label="gene", label.select=unique(c(data[data$pval < 0.05,"gene"][1:10],data[order(data$d_mutfreq,decreasing=TRUE),"gene"][1:5],data[order(data$t_mutfreq,decreasing=TRUE),"gene"][1:8]))) + 
geom_point(data=data[data$pval <0.05,],size=1.5,stroke=0.8,pch=8,fill=NA) + 
scale_size_continuous(range = c(2, 10),limits=c(0,3)) + scale_fill_gradientn(colors=carto_pal(7,"ag_GrnYl")) + geom_abline(slope=1,intercept=0,linetype="dashed") + theme(aspect.ratio=1)
dev.off()
