#!/usr/bin/Rscript
# use ASXL1 vs TET2 definitions at time of diagnosis to guide subsequent labels
# NOT from each stage
library(ggplot2)
library(ggpubr)
...
# can only have ASXL1 or TET2 mutations; mutually exclusive
dataDN2 = dataDN[dataDN$ASXL1 + dataDN$TET2 == 1,]  # 30 vs 18
dataDN2$DTA = dataDN2$TET2 - dataDN2$ASXL1 # -1: ASXL1; +1: TET2
...
colnames(mydf) = c("gene","beta_TET2_vs_ASXL1","lowCI","hiCI","pval")
mydf = as.data.frame(mydf)
mydf = mydf[! mydf$gene %in% c("TET2","ASXL1"),]
mydf = mydf[order(mydf$pval),]

write.table(mydf,"TET2-vs-ASXL1.exclusive.diagnosis.logistf.txt",sep="\t",row.names=FALSE)

####################
#### get mut frequencies in each group

# mutually exclusive
datad = dataDN2[dataDN2$DTA == 1,] # TET2
datat = dataDN2[dataDN2$DTA == -1,] # ASXL1

d1 = cbind(rownames(datad),datad$patient,datad$stage,"TET2mut")
d2 = cbind(rownames(datat),datat$patient,datat$stage,"ASXL1mut")
sampled2 = as.data.frame(rbind(d1,d2))
write.table(sampled2,"diagnosis.any.TET2-ASXL1.samples.txt",sep="\t",row.names=FALSE)
...
data = read.table("TET2-vs-ASXL1.exclusive.diagnosis.logistf.txt",sep="\t",header=TRUE,row.names=1)
...
pdf("diagnosis.TET2-vs-ASXL1.mutfreqs.logistf.pdf",height=5,width=4,useDingbats=FALSE)
ggscatter(data,x="d_mutfreq", y="t_mutfreq",fill="sumPct",shape=21,color="black",repel=TRUE,alpha=0.75,size="nlp",xlim=c(0,60),ylim=c(0,40),label="gene", label.select=unique(c(data[data$pval < 0.05,"gene"][1:10],data[order(data$d_mutfreq,decreasing=TRUE),"gene"][1:5],data[order(data$t_mutfreq,decreasing=TRUE),"gene"][1:8]))) + 
...
labels = read.table("diagnosis.any.TET2-ASXL1.samples.txt",sep="\t",header=TRUE)

# filter dataREL to REL1 samples from patients with a TET2 or ASXL1 exclusive mutation at diagnosis (so TET2 / ASXL1 may actually be *lost* at time of REL1)
...
dataREL[dataREL$DTA == "ASXL1mut","DTA"] = -1
...
write.table(mydf,"TET2-vs-ASXL1.exclusive.REL1.logistf.txt",sep="\t",row.names=FALSE)
...
data = read.table("TET2-vs-ASXL1.exclusive.REL1.logistf.txt",sep="\t",header=TRUE,row.names=1)
fdata = fdata[rownames(fdata) %in% rownames(data),]
fdata = fdata[match(rownames(data),rownames(fdata)),]
all(rownames(fdata) == rownames(data))
...
pdf("REL1.TET2-vs-ASXL1.mutfreqs.logistf.pdf",height=5,width=4,useDingbats=FALSE)
ggscatter(data,x="d_mutfreq", y="t_mutfreq",fill="sumPct",shape=21,color="black",repel=TRUE,alpha=0.75,size="nlp",xlim=c(0,70),ylim=c(0,60),label="gene", label.select=unique(c(data[data$pval < 0.05,"gene"][1:10],data[order(data$d_mutfreq,decreasing=TRUE),"gene"][1:5],data[order(data$t_mutfreq,decreasing=TRUE),"gene"][1:8]))) + 
geom_text(data=data[data$pval <0.05,],size=8,hjust=0.5,vjust = 0.75,color="black",label="*") + 
scale_size_continuous(range = c(2, 10),limits=c(0,3)) + scale_fill_gradientn(colors=carto_pal(7,"ag_GrnYl")) + geom_abline(slope=1,intercept=0,linetype="dashed") + theme(aspect.ratio=1)
dev.off()
